...
quark-cli torrent test --client-id qb2        # 测试指定客户端
```

### 查看下载列表

```bash
quark-cli torrent list
quark-cli torrent list --filter downloading    # 按状态过滤
quark-cli torrent list --category Anime        # 按分类过滤
```

| 选项 | 说明 |
|------|------|
| `--filter` | 状态：`all` / `downloading` / `seeding` / `completed` / `paused` / `active` |
| `--category` | 分类过滤 |
| `--limit` | 显示条数（默认 20） |
| `--client-id` | 客户端 ID |
...
quark-cli torrent add "https://example.com/file.torrent"
quark-cli torrent add "magnet:..." --save-path /downloads/movie --category Movie
```
...
> **预留**：当前仅支持 qBittorrent。Transmission（RPC）和 aria2（JSON-RPC）已在配置中预留，如有需求可开发。
...
RSS 规则支持 `--action torrent`，将匹配到的 `.torrent` 或磁力链接推送到 qBittorrent。

```bash
# Mikan 番剧追更 → qBittorrent
quark-cli rss rule add mikan_feed \
  --name "追番4K" \
  --match "葬送的芙莉莲" \
  --quality "1080p|2160p" \
  --action torrent \
  --link-type enclosure \
  --torrent-save-path "/downloads/anime" \
  --torrent-category "Anime" \
  --torrent-tags "rss,auto"

# 磁力链接 RSS → qBittorrent
quark-cli rss rule add nyaa_feed \
  --name "字幕组" \
  --match "Frieren" \
  --action torrent \
  --link-type magnet
...
| 选项 | 说明 |
|------|------|
| `--torrent-save-path` | qB 下载路径（覆盖客户端默认） |
| `--torrent-category` | qB 分类 |
| `--torrent-tags` | qB 标签（逗号分隔） |
| `--torrent-client` | 客户端 ID（多实例场景） |
| `--torrent-paused` | 添加后暂停 |
